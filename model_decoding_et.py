...
class T5Translator(nn.Module):
    def __init__(self, pretrained_layers, in_feature = 840, decoder_embedding_size = 1024, additional_encoder_nhead=8, additional_encoder_dim_feedforward = 2048, fixation_embedding_dim=128):
        super(T5Translator, self).__init__()
        
        self.pretrained = pretrained_layers

        self.tokenizer = T5Tokenizer.from_pretrained("t5-large")
         # Эмбеддинги для квантилей TRT
        self.trt_embedding = nn.Embedding(10, fixation_embedding_dim)  # 10 квантилей TRT
        
        # Отдельный энкодер для TRT
        self.trt_encoder_layer = nn.TransformerEncoderLayer(
            d_model=fixation_embedding_dim,
            nhead=4,  # Меньше голов, так как размерность TRT меньше
            dim_feedforward=fixation_embedding_dim * 4,
            batch_first=True
        )
        self.trt_encoder = nn.TransformerEncoder(self.trt_encoder_layer, num_layers=2)  # Меньше слоёв для TRT
        
        # Энкодер для объединённых эмбеддингов (ЭЭГ + TRT)
        self.combined_encoder_layer = nn.TransformerEncoderLayer(
            d_model=in_feature + fixation_embedding_dim,
            nhead=additional_encoder_nhead,
            dim_feedforward=additional_encoder_dim_feedforward,
            batch_first=True
        )
        self.combined_encoder = nn.TransformerEncoder(self.combined_encoder_layer, num_layers=6)
        
        # Линейный слой
        self.fc1 = nn.Linear(in_feature + fixation_embedding_dim, decoder_embedding_size)

    def addin_forward(self, input_embeddings_batch, input_masks_invert, trt_quantile_ids):
        """input_embeddings_batch: [batch_size, seq_len, 840]"""
        """input_masks_invert: [batch_size, seq_len], 1 is masked, 0 is not masked"""
        """trt_quantile_ids: [batch_size, seq_len]"""
        
        # Получаем эмбеддинги TRT
        trt_emb = self.trt_embedding(trt_quantile_ids)  # [batch_size, seq_len, fixation_embedding_dim]
        
        # Обрабатываем TRT отдельным энкодером
        trt_emb = self.trt_encoder(trt_emb, src_key_padding_mask=input_masks_invert)  # [batch_size, seq_len, fixation_embedding_dim]
        
        # Конкатенируем ЭЭГ и TRT
        combined_embeddings = torch.cat([input_embeddings_batch, trt_emb], dim=-1)  # [batch_size, seq_len, 840 + fixation_embedding_dim]
        
        # Обрабатываем объединённые эмбеддинги
        encoded_embedding = self.combined_encoder(combined_embeddings, src_key_padding_mask=input_masks_invert)
        
        # Линейное преобразование
        encoded_embedding = F.relu(self.fc1(encoded_embedding))  # [batch_size, seq_len, 1024]
        return encoded_embedding
    ...
    def generate(
        ...
        encoded_embedding = self.addin_forward(input_embeddings_batch, input_masks_invert, trt_quantile_ids)

        input_ids = self.tokenizer("transcribe in English: ", return_tensors="pt").input_ids.to(encoded_embedding.device)
        task_embedding = self.pretrained.shared(input_ids).to(encoded_embedding.device)
        task_embedding = task_embedding.repeat(encoded_embedding.size(0), 1, 1).to(encoded_embedding.device)
    ...
    def forward(self, input_embeddings_batch, input_masks_batch, input_masks_invert, target_ids_batch_converted, trt_quantile_ids=None):
        encoded_embedding = self.addin_forward(input_embeddings_batch, input_masks_invert, trt_quantile_ids)
