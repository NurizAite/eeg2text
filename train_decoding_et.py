...
from data_et import ZuCo_dataset
from model_decoding_et import BrainTranslator, BrainTranslatorNaive, T5Translator
...
def train_model(dataloaders, device, model, criterion, optimizer, scheduler, num_epochs=25, checkpoint_path_best='./checkpoints/decoding/best/temp_decoding.pt', checkpoint_path_last='./checkpoints/decoding/last/temp_decoding.pt'):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'dev']:
            ...
            for input_embeddings, seq_len, input_masks, input_mask_invert, target_ids, target_mask, sentiment_labels, trt_quantile_ids in tqdm(dataloaders[phase]):
                input_embeddings_batch = input_embeddings.to(device).float()
                input_masks_batch = input_masks.to(device)
                input_mask_invert_batch = input_mask_invert.to(device)
                target_ids_batch = target_ids.to(device)
                trt_quantile_ids_batch = trt_quantile_ids.to(device).long()
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    seq2seqLMoutput = model(input_embeddings_batch, input_masks_batch, input_mask_invert_batch, target_ids_batch, trt_quantile_ids_batch)
...
if __name__ == '__main__':
    ...
    fixation_embedding_dim = 32

    # ... код для загрузки данных и даталоадеров ...
    ...
    if model_name == 'BrainTranslator':
        ...
        model = T5Translator(
            ...
            fixation_embedding_dim=fixation_embedding_dim
        )
