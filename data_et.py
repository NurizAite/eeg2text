...
def get_input_sample(sent_obj, tokenizer, eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'], max_len = 56, add_CLS_token = False, test_input="noise", quantiles = None):
    def get_word_et_data(word_obj):
        trt = word_obj['et_data']['TRT']
        trt_id = np.digitize(trt, quantiles[:-1]) -1 if trt > 0 else 0
        trt_id = min(trt_id, len(quantiles) - 2)
        return trt_id
    ...
    for word in sent_obj['word']:
        ...
        trt_id = get_trt_quantile_id(word, trt_quantiles)
        trt_quantile_ids.append(trt_id)

    # pad to max_len
    while len(word_embeddings) < max_len:
        word_embeddings.append(torch.zeros(105*len(bands)))
        trt_quantile_ids.append(0)
    ...
    # Добавляем индексы квантилей TRT
    input_sample['trt_quantile_ids'] = torch.tensor(trt_quantile_ids, dtype=torch.long)  # [max_len]
...
class ZuCo_dataset(Dataset):
    def __init__(self, input_dataset_dicts, phase, tokenizer, subject = 'ALL', eeg_type = 'GD', bands = ['_t1','_t2','_a1','_a2','_b1','_b2','_g1','_g2'], setting = 'unique_sent', is_add_CLS_token = False, test_input='noise'):
        ...
        self.trt_quantiles = self._compute_trt_quantiles(input_dataset_dicts)
                ...
                if phase == 'train':
                    ...
                    for key in subjects:
                        for i in range(train_divider):
                            input_sample = get_input_sample(input_dataset_dict[key][i],self.tokenizer,eeg_type,bands = bands, add_CLS_token = is_add_CLS_token, test_input=test_input, trt_quantiles=self.trt_quantiles)
                ...
                if phase == 'train':
                    ...
                    for i in range(total_num_sentence):
                        for key in ['ZAB', 'ZDM', 'ZGW', 'ZJM', 'ZJN', 'ZJS', 'ZKB', 'ZKH','ZKW']:
                            input_sample = get_input_sample(input_dataset_dict[key][i],self.tokenizer,eeg_type,bands = bands, add_CLS_token = is_add_CLS_token, trt_quantiles=self.trt_quantiles)
                            if input_sample is not None:
                                self.inputs.append(input_sample)
                if phase == 'dev':
        ...
        print('[INFO]TRT quantile IDs tensor size:', self.inputs[0]['trt_quantile_ids'].size())

        print()
    def _compute_trt_quantiles(self, input_dataset_dicts):
        all_trt = []
        for input_dataset_dict in input_dataset_dicts:
            for subject in input_dataset_dict:
                for sent in input_dataset_dict[subject]:
                    for word in sent['word']:
                        trt = word.get('TRT', 0.0)
                        if trt > 0:
                            all_trt.append(trt)
        
        # Вычисляем 10 квантилей
        num_quantiles = 10
        quantiles = np.quantile(all_trt, np.linspace(0, 1, num_quantiles + 1)) if all_trt else np.zeros(num_quantiles + 1)
        print("TRT quantiles:", quantiles)
        return quantiles
    ...
    def __getitem__(self, idx):
        ...
        return (
            ...
            input_sample['trt_quantile_ids']  # [max_len]
        )
...
if __name__ == '__main__':
    ...
    if check_dataset == 'ZuCo':
        ...
        dataset_path_task1 = '/home/dataset/ZuCo/task1-SR/pickle/task1-SR-dataset-with-tokens_6-25.pickle' 
        with open(dataset_path_task1, 'rb') as handle:
            whole_dataset_dicts.append(pickle.load(handle))

        dataset_path_task2 = '/home/dataset/ZuCo/task2-NR/pickle/task2-NR-dataset-with-tokens_7-10.pickle' 
        ...
        dataset_path_task2_v2 = '/home/dataset/ZuCo/task2-NR-2.0/pickle/task2-NR-2.0-dataset-with-tokens_7-15.pickle' 
